...
This script uses src/utils/audioPaths.ts to enumerate every generated audio
...
(
  cd "$REPO_ROOT"
  npx --yes tsx - "$PARK_NAME" "$BROWSER_FAMILY" "$VARIANT_MODE" <<'EOF'
import stateParks from './src/data/stateParks.json' with { type: 'json' };
import { formatParkSlug, getParkAudioVariants, pickSoundPath } from './src/utils/audioPaths.ts';
...
if [[ "$VARIANT_MODE" == "generated" ]]; then
  echo "Mode: variants generated by current audioPaths.ts logic"
else
