...
Good-Earth-2-001 matches `Edits/ICEBrk-OctoMic F8n_Good Earth State Park-Big Sioux River, ice breaking_TAC_SD State Parks.wav`. Its windows align at 151.9910 and 151.9905 seconds, with correlations 0.6181 and 0.6151. A nominal cut at 152 seconds is a strong inference, not a sample-exact recipe. All four Good Earth deliveries align at nominal 32, 92, 152 and 212 seconds in this same ice-breaking session. The roughly 9 ms difference includes encoder/filter timing. The archived eight-channel session contains capsule signals and must undergo calibrated A-to-B encoding before the legacy delivery split. Directly cutting it and converting to FLAC would produce the wrong signal format. **rl-74x.1** has this evidence. A practical repair uses the intact AAC delivery twin, decoded to 16-bit PCM and wrapped in FLAC. This restores all 2,646,016 samples without pretending to recover a lossless master. The embedded comment and repair manifest label its lossy source. A calibrated master rerender was proposed as separate work (rl-dz0) and has been decided against; the AAC-derived FLAC is the archive of record for this one recording. The reason is that the render cannot be reproduced. The project that encoded the corpus in January 2024 is not on this machine — `conversion/conversion.RPP` references two edits where the deliveries need thirty-three, and no other REAPER project with more than a handful of VVOctoEncode instances exists outside 'Veins of the Earth', which is a different piece from 2025. Rendering the ICEBrk session through VVOctoEncode as it stands reproduces W at r=0.94 and nothing else: first order is uncorrelated with the corpus and second order is 9 to 24 dB out depending on which of the three OM-0368 calibrations is chosen, none of which is the one used. See rl-dqc.7.2 for the full elimination. So a rerender would not restore the 2024 encoding; it would substitute a different one. And because `pickAssetFamily` serves FLAC to Safari and AAC elsewhere, rerendering only the lossless family would split the two families for this recording — iPhone walkers hearing a 2026 encode and everyone else the 2024 one. Rerendering both would replace a delivery that is correct with one that is merely current. What the walker gets today is the full 2,646,016 samples of the right recording at the right offset, carrying AAC generation loss in the family that is nominally lossless. That is a purity cost in one file out of 102, and it is smaller than either alternative.
