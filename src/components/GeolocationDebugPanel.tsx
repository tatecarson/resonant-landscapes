        ...
        <div className="pointer-events-auto fixed left-3 top-[max(0.75rem,env(safe-area-inset-top))] z-[60] w-[min(20rem,calc(100vw-1.5rem))] overflow-hidden rounded-[1.6rem] border border-edge/15 bg-ground/[78%] p-3 text-[11px] leading-4 text-ink-muted shadow-console backdrop-blur-md">
            ...
            <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                    <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.28em] text-ink-muted">Field Console</p>
                    <p className="truncate font-display text-xl leading-none text-ink">{window.location.pathname}</p>
                </div>
                <button
                    type="button"
                    onClick={() => setIsCollapsed((current) => !current)}
                    className="rounded-full border border-edge/15 bg-white/45 px-3 py-1.5 font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-ink-muted transition hover:border-edge/30 hover:bg-white/65"
                >
                ...
                <div className="mt-3 space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                        <div className="rounded-2xl bg-white/[42%] px-3 py-2">
                            <p className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Context</p>
                            <p className="mt-1 font-semibold text-ink">{audioState}</p>
                        </div>
                        <div className="rounded-2xl bg-white/[42%] px-3 py-2">
                            <p className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Park</p>
                    ...
                    <div className="rounded-[1.2rem] border border-edge/10 bg-white/[38%] px-3 py-3">
                        <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1">
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Unlocked</span>
                            <span className="font-semibold text-ink">{isAudioUnlocked ? "yes" : "no"}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Loading</span>
                            <span className="font-semibold text-ink">{isLoading ? "yes" : "no"}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Playing</span>
                            <span className="font-semibold text-ink">{isPlaying ? "yes" : "no"}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">UI</span>
                            <span className="font-semibold text-ink">{window.__audioDebug?.uiStatus ?? "n/a"}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Source</span>
                            <span className="font-semibold text-ink">{hasSourceNode ? "present" : "missing"}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Buffers</span>
                            <span className="font-semibold text-ink">{hasBuffers ? "loaded" : "empty"}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Duration</span>
                            <span className="font-semibold text-ink">{bufferDuration ? `${bufferDuration.toFixed(2)} s` : "n/a"}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Channels</span>
                            <span className="font-semibold text-ink">{bufferChannels ?? "n/a"}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Cache</span>
                            <span className="font-semibold text-ink">{audioDebug?.cacheEntries ?? "n/a"}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Event</span>
                            <span className="font-semibold text-ink">{audioDebug?.lastEvent ?? "n/a"}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Cache hit</span>
                            <span className="font-semibold text-ink">{cacheHitLabel}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Geo</span>
                            <span className="font-semibold text-ink">{debugPermission}</span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Coords</span>
                    ...
                    {!isAudioUnlocked && (
                        <button
                            ...
                            className="rounded-full border border-edge/15 bg-edge px-3 py-2 font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-ground transition hover:bg-ink"
                        >
                            Unlock Audio
                        </button>
                    )}
                    {renderDebugEntries.length > 0 && (
                        <div className="rounded-[1.2rem] bg-surface/[72%] px-3 py-3 text-[10px] text-ink-muted">
                            <p className="font-mono text-[9px] font-semibold uppercase tracking-[0.22em] text-ink-muted">Render counts</p>
